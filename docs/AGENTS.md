...
本文件定义 AI 智能体与人工开发者在本项目中的协作边界、执行流程、风险升级机制与交接要求。

- 适用对象：Claude、Copilot、Cursor、IDE 内置助手、脚本化代码生成工具、人工开发者
- 适用范围：需求分析、文档编写、代码实现、测试补充、缺陷修复、发布准备
- 规范定位：本文件只描述“如何协作”，不重复 Java、DDD、API、数据库、Git、发布细则
...
| 区域 | 说明 | 默认策略 |
|------|------|----------|
| 绿区 | 应用层、接口层、测试、文档、非敏感配置 | 可直接修改 |
| 黄区 | 领域模型、仓储接口、基础设施实现、共享组件 | 先说明影响，再修改 |
| 红区 | 安全核心、数据库迁移脚本、生产配置、CI 发布脚本 | 必须人工确认 |

补充约束：

- 修改 `domain`、`repository`、`db/migration`、`application-prod.yml`、`.github/workflows/` 前必须明确说明风险
- 不得因重构方便而绕过既有安全校验、审计日志、异常处理链路
- 不得擅自删除已有测试来“让构建通过”
...
- 先阅读现有实现，再决定新增还是复用
- 优先做最小可验证改动，避免一轮任务内同时改动架构、接口和持久化
- 说明默认假设，例如“当前以 DDD 四层结构为基线”
...
- 在不说明风险的情况下修改数据库迁移脚本或生产配置
- 用“为尽快完成任务”为理由跳过测试、审查、自检
