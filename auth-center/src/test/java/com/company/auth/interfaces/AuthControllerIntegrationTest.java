...
class AuthControllerIntegrationTest extends BaseJdbcWebIntegrationTest {
    ...
    void shouldRejectBadPasswordAndRecordFailureLog() throws Exception {
        mockMvc.perform(post("/api/v1/auth/login")
            ...
            .andExpect(jsonPath("$.message", anyOf(
                is("登录名或密码错误"),
                is("Login name or password is incorrect"))));
    ...
    void shouldRejectDisabledUserAndRecordFailureLog() throws Exception {
        mockMvc.perform(post("/api/v1/auth/login")
            ...
            .andExpect(jsonPath("$.message", anyOf(
                is("当前账号已被禁用"),
                is("Current account is disabled"))));
    ...
    void shouldExposeCurrentUserAndAccessCodesAndRejectRevokedToken() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/auth/me")
            ...
            .andExpect(jsonPath("$.message", anyOf(
                is("访问令牌已失效或不可用"),
                is("Access token is revoked or unavailable"))));
    }
    ...
    void shouldRequireAuthenticationForProtectedEndpoints() throws Exception {
        mockMvc.perform(get("/api/v1/auth/me"))
            ...
            .andExpect(jsonPath("$.message", anyOf(
                is("缺少 Authorization Bearer 令牌"),
                is("Authorization bearer token is required"))));
    ...
    void shouldRejectInvalidBearerToken() throws Exception {
        mockMvc.perform(get("/api/v1/auth/me")
            ...
            .andExpect(jsonPath("$.message", anyOf(
                is("访问令牌格式无效"),
                is("Access token format is invalid"))));
    }
