...
- M5 archive object pagination is served by `bl-center` for sibling frontend `/operation-support/archive`: `GET /api/v1/archive-objects` requires `objectType=APPLICATION_FORM|EMBEDDING_BOX|SLIDE`, accepts optional `keyword` plus `page/size`, reuses `PERM_M5_ARCHIVE_QUERY`, and returns all queryable application forms, wax blocks, or slides with archive/loan status fields. It must source objects from real business tables and keep `GET /api/v1/archive-records/search` available for archive-record/history consumers.
