...
$report.Add("| Check Item | Basis | Status | Evidence | Suggested Action |")
$report.Add("| --- | --- | --- | --- | --- |")
$report.Add("| Structural hotspots | docs/rules/AI_CODE_HEALTH_CORE.md; docs/rules/CODING_RULES.md; docs/reports/largest-files-report.md | $structuralStatus | $(if ($largest.Available) { "Java files over 300 lines: $($largest.JavaOver300); Java files over 500 lines: $($largest.JavaOver500)." } else { 'largest-files-report.md is unavailable, so hotspot counts could not be refreshed.' }) | Prioritize breaking up persistence, service, and controller files with heavy line counts. |")
$report.Add("")
...
$report.Add("| Check Item | Basis | Status | Evidence | Suggested Action |")
$report.Add("| --- | --- | --- | --- | --- |")
$report.Add("| Generic filenames | docs/rules/AI_CODE_HEALTH_CONTRACTS.md; docs/rules/CODING_RULES.md | $genericFilesStatus | $(if ($genericNameHits.Count -gt 0) { 'Matched generic basenames: ' + (Convert-ToMarkdownCell (($genericNameHits | Select-Object -First 5) -join ', ')) + '.' } else { 'No tracked files matched generic basenames such as utils, common, helper, helpers, tools, tmp, or temp.' }) | Keep file and module names anchored in domain language. |")
$report.Add("| TODO / FIXME noise | docs/rules/CODING_RULES.md; docs/rules/AI_CODE_HEALTH_CORE.md | $todoStatus | $(if ($scans.TodoHits.Count -gt 0) { 'TODO/FIXME hits: ' + (Convert-ToMarkdownCell (($scans.TodoHits | Select-Object -First 5) -join '; ')) + '.' } else { 'The scan only matched the domain constant TODO_TASK; no stray TODO/FIXME markers were found.' }) | Keep domain constants distinct from comment-based follow-up markers. |")
$report.Add("")
$report.Add("## Errors and Encoding")
$report.Add("")
$report.Add("| Check Item | Basis | Status | Evidence | Suggested Action |")
$report.Add("| --- | --- | --- | --- | --- |")
$report.Add("| Implicit charset conversions | docs/rules/CODING_RULES.md; docs/rules/AI_CODE_HEALTH_CONTRACTS.md | $encodingStatus | $(if ($scans.EncodingHits.Count -gt 0) { 'Potential hotspots: ' + (Convert-ToMarkdownCell (($scans.EncodingHits | Select-Object -First 5) -join '; ')) + '.' } else { 'No raw new String(byte[]), FileReader, or FileWriter usage was found in the scanned source tree.' }) | Keep charsets explicit, especially on import, export, and log-writing paths. |")
$report.Add("| Empty catch / silent exception swallowing | docs/rules/CODING_RULES.md; docs/rules/AI_CODE_HEALTH_CORE.md | $emptyCatchStatus | $(if ($scans.EmptyCatchHits.Count -gt 0) { 'Empty catch hits: ' + (Convert-ToMarkdownCell (($scans.EmptyCatchHits | Select-Object -First 5) -join '; ')) + '.' } else { 'No empty catch blocks were found by the regex scan.' }) | Keep exceptions structured and propagate them at the right layer. |")
$report.Add("")
$report.Add("## Notes")
$report.Add("")
